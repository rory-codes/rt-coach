...
PHASE_WARMUPS = {
    ...
    "strength":    "RAMP warm-up + barbell ramps",
...
def calc_max_hr(age: Optional[float]) -> Optional[int]:
    ...
    return int(round(220 - age)) if age is not None else None

def calc_zones(age: Optional[float], rhr: Optional[float]) -> List[Dict]:
    age = _clamp(age, 1, 120); rhr = _clamp(rhr, 20, 150)
    max_hr = calc_max_hr(age)
    if max_hr is None or rhr is None or rhr >= max_hr: return []
    hrr = max_hr - rhr
    out = []
    for name, lo, hi in ZONES_DEF:
        out.append({
            "name": name, "lo_pct": lo, "hi_pct": hi,
...
def estimate_1rm_from_10rm(tenrm: Optional[float]) -> Optional[float]:
    if tenrm is None: return None
    try: tenrm = float(tenrm)
    except (TypeError, ValueError): return None
    if tenrm <= 0: return None
    return tenrm * (1 + 10.0/30.0)  # Epley for 10 reps
...
def weekly_cardio_sessions(experience: str) -> int:
    return {"beginner": 2, "intermediate": 3, "experienced": 4}.get((experience or "").lower(), 3)
...
def cardio_minutes_by_phase(phase: str, experience: str) -> Tuple[int, int]:
    ...
    if phase == "endurance":
        return {"beginner": 25, "intermediate": 35, "experienced": 45}[e], 1
    if phase == "hypertrophy":
        return {"beginner": 20, "intermediate": 30, "experienced": 35}[e], 2
    if phase == "strength":
        return {"beginner": 15, "intermediate": 20, "experienced": 25}[e], 2
    return {"beginner": 15, "intermediate": 20, "experienced": 20}[e], 3
...
def _exercise_rows_for_day(day_idx: int, phase: str, one_rm_map: Dict[str, Optional[float]]) -> List[Dict]:
    ...
    for ex in exs:
        ...
        rows.append({
            "name": ex, "sets": pres["sets"], "reps": pres["reps"], "pct_1rm": pct,
            "load_kg": load, "rpe": pres["rpe"], "rest_s": pres["rest_s"], "tempo": pres["tempo"],
            "human": _compose_exercise_line(ex, pres["sets"], pres["reps"], pct, pres["rpe"], pres["rest_s"], pres["tempo"], load)
        })
    return rows
...
def build_plan(experience: str, goal: str, metrics: Metrics) -> Dict:
    ...
    for w in range(12):
        ...
        out_weeks.append({"week": w + 1, "phase": phase, "days": days})

    return {
        "meta": {"experience": experience, "goal": goal, "age": metrics.age, "rhr": metrics.rhr},
        "zones": zones,
        "weeks": out_weeks,
    }
...
def plan_to_rows(plan_json: Dict) -> List[Dict[str, str]]:
    ...
    for wk in plan_json.get("weeks", []):
        ...
        for d in wk.get("days", []):
            cardio = d.get("cardio", {})
            zone = cardio.get("zone"); bpm = cardio.get("bpm")
            if zone and bpm:
